...
module.exports = async (req, res, next) => {
    try{
        ...
        if(req.body.appointment_date == undefined || req.body.appointment_date == ""){
            console.log("Appointment date not selected for G Test yet!");
        }
        else if(req.body.selected_time == undefined || req.body.selected_time == null){
            console.log("Time slot not selected for G Test");
            let appointment_date = req.body.appointment_date;
            console.log("Appointment date selected for G Test: " + appointment_date);
            ...
            console.log("Both appointment date and time selected for G Test");
            let appointment_date = req.body.appointment_date;
            let appointment_object = await Appointment.findOne({appointment_date: appointment_date});
            ...
            if(user_object.appointment_id == null){
                ...
                await User.updateOne({username: user_object.username}, {appointment_id: selected_slot_object[0]._id, testType: "G"})
        ...
        console.log("Error occurred in selecting the appointment selection: in G " + error);
    }
    next();
}